...
rule extract_egfr_g719:
    input:
        "jsnpmania/{sample}.variations"
    output:
        "reports/extracted_sample_info/{sample}.egfr_g719.tsv"
    params:
        min_depth = 5,
        tissue = lambda wildcards: samples['tissue'][wildcards.sample],
        experiment = lambda wildcards: samples['experiment'][wildcards.sample]
    log:
        "logs/reports/extracted_sample_info/{sample}.egfr_g719.log"
    run:
        extract_egfr(
            input[0],
            output[0],
...
rule combine_egfr_t790m:
    input:
        expand("reports/extracted_sample_info/{sample}.egfr_t790m.tsv", sample=extract_lung_samples())
    output:
        "reports/EGFR_T790M.allSamples.ampliconmapped.txt"
    run:
        if len(input) > 1:
...
rule combine_egfr_g719:
    input:
        expand("reports/extracted_sample_info/{sample}.egfr_g719.tsv", sample=extract_lung_samples())
    output:
        "reports/EGFR_G719.allSamples.ampliconmapped.txt"
    run:
        if len(input) > 1:
            shell("""cat {input} | awk 'BEGIN{{NUM=0}}{{if(!/^#/){{print($0);}}else if(NUM == 0){{print($0);}}NUM=NUM+1;}}' > {output}""")
        elif len(input) == 1:
